//Largest product in a series
//Problem 8

//The four adjacent digits in the 1000-digit number that have 
// the greatest product are 9 × 9 × 8 × 9 = 5832.


#include <iostream>     // std::cout, std::end
#include "eulerNumbers.h"
#include <cstring>
#include <string>

using namespace std;

int main() {
	string strNum = "";
	char * num = new char[strNum.size() + 1];
	strcpy(num, strNum.c_str());
	//ASSERT: array num contains given number	
	int numDigits = 0;
	cout << "Enter number of digits, problem calls for 13" << endl;
	cin >> numDigits;
	unsigned long int grProduct = 1; //current largest product
	int grIndex = 0;//holds index for first of the <numDigits. factors of the greatest product
	unsigned long int tempProduct = 1; //current product
	int tempIndex = 0;//index for first of 13 for temp product
	for(int i = 0; (i < (strNum.size() - numDigits)); i++) {
		tempIndex = 0;
		tempProduct = 1;		
		while(tempIndex < numDigits) {	
			tempProduct = tempProduct * ((int)num[(i + tempIndex)] - '0');
			tempIndex++;
		}//end while
		if(tempProduct > grProduct) {
			grProduct = tempProduct;
			grIndex = i;
		}
	}//end for
	cout << "Largest product of " << numDigits << " consecutive integers is ";
	cout << grProduct << endl;
	cout << "Which starts at index " << grIndex << endl;	
	delete [] num;
	return(0);
}
